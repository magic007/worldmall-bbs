...
class plugin_milu_pick {
	...
	function global_header() {
		...
		if( ( $set['cron_open'] == 1 || $set['is_timing'] == 1 ) && strexists(strtolower($_G['setting']['plugins']['version']['milu_pick']), 'vip')){
...
class plugin_milu_pick_forum extends plugin_milu_pick {
	...
	function post_bottom_output() {
		if($this->output) return $this->script;
		...
		if($detailstatus){//显示详细的在线会员
			if(!$setting){//缓存起来
				...
				foreach($member_list as $k => $v){
					...
					$memberlist = array('uid' => $v['uid'], 'username' => $v['username'], 'groupid' => $groupid, 'invisible' => $invisible, 'icon' => $icon, 'action' => 2, 'lastactivity' => $lastactivity);
	...
	function viewthread_bottom_output(){
		...
		foreach($postlist as $pid => $post) {
			if($post['first'] == 1){
				...
				$postlist[$pid]['subject'] = $_G['forum_thread']['subject'] = $seo_arr['title'];
			}
			$postlist[$pid]['message'] = preg_replace("/<br \/>|<br>/e", "\$this->_jammer()", $postlist[$pid]['message']);
		}
	}
