...
class pick{
	...
	function _ini_config($pid = 0, $is_cron = 0){
		...
		if(!VIP) $p_arr['reply_is_extend'] = $p_arr['reply_rules'] = '';
	...
	function parse_page($type = 'content', $content = ''){
		if(GBK){
			//if(ischinese($this->now_url) == 'encn') $this->now_url = iconv("gbk", "UTF-8", $this->now_url);
		}
	...
	function get_start_url(){
		...
		if($this->p_arr['page_fiter'] == 1  && $this->now_url_arr){//开启了过滤网址功能
			if($this->p_arr['page_url_other']) $this->now_url_arr = array_merge(format_wrap($this->p_arr['page_url_other']), $this->now_url_arr);
		}
	...
	function get_reply($content, $reply_num = 0){
		...
		if($this->p_arr['reply_get_type'] == 1){
			...
			$re = pregmessage($content, $this->p_arr['reply_rules'], 'reply', -1);
			$reply_arr = $re[0];
	...
	function article_public($arr){
		...
		//发布时间大于当前时间，放入定时发布中（此功能还待完善）
		if($arr['public_time'] > $_G['timestamp'] && $is_timing == 1 && $this->aid){
			...
			$timing_setarr = array('public_type' => $this->p_arr['public_type'], 'data_id' => $this->aid, 'content_type' => 1, 'public_dateline' => $arr['public_time'], 'pid' => $this->pid, 'public_info' => serialize($timing_public_arr));
