...
function pick_import(){
	...
	if(!submitcheck('submit')) {
		num_limit('strayer_picker', 30, 'p_num_limit');
		$info['header'] = pick_header_output($header_config, $head_url);
		return $info;
	}else{
...
function pick_import_data($arr, $pick_cid){
	...
	$arr['pick'] = pstripslashes($arr['pick']);
	...
	$insert_id = DB::insert('strayer_picker', $arr['pick'], TRUE);//导入采集器
	if($arr['rules']){
		$rules_arr = $arr['rules'];	
...
function show_pick_format($info){	
	if(strexists(DISCUZ_VERSION, '2.5')) $info = dstripslashes($info);
	$info['rules_var'] = unserialize($info['rules_var']);
	$info['many_page_list'] = unserialize($info['many_page_list']);
	$info['title_filter_rules'] = unserialize($info['title_filter_rules']);
	$info['content_filter_rules'] = unserialize($info['content_filter_rules']);
	$info['reply_filter_rules'] = unserialize($info['reply_filter_rules']);
	$info['content_filter_html'] = unserialize($info['content_filter_html']);
	$info['reply_filter_html'] = unserialize($info['reply_filter_html']);
	$info['public_class'] = unserialize($info['public_class']);
	$info['public_uid_group'] = unserialize($info['public_uid_group']);
	$info['reply_uid_group'] = unserialize($info['reply_uid_group']);
