...
function media_format($text, $url = ''){
	...
	$pregfind = array(
		"/<script.+'width',\s+'(\d+)',\s+'height',\s+'(\d+)'.+'src',\s+'([^\']+)'.+<\/script>/eiU",
...
function mediatag($attributes, $page_url, $type = 'bbs') {
	...
	preg_match_all('/(src|width|flashvars|height)=([\"|\'])?(.*?)(?(2)\2|\s)/is', dstripslashes($attributes), $matches);
...
//发表回复
function post($str, $args = array()){
	global $_G;
	extract($args);
	if(!$str || !$cookie) return $str;
	preg_match_all('/<div class="locked">.*?<a href="(.*?)" >.*?<\/a><\/div>/is', $str, $arr, PREG_SET_ORDER);
	if(!$arr) return $str;
	$lock_str = $arr[0][0];
	$url = $arr[0][1];
	if(!$url) return $str;
	$url = convert_url($url);
	$url_info = explode('forum.php?mod=post',$url);
	$host = $url_info[0];
	$a = parse_str($url_info[1]);
	$snoopy_args['cookie'] = $cookie;
	$snoopy = get_snoopy_obj($snoopy_args);
	...
	if(!$html) return $str;
	foreach($html->find('form[action*=forum.php] input[type=hidden]') as $k => $v) {
		$value_arr[$v->attr['name']] = $v->attr['value'];
	}
	$html->clear();
	unset($html);
	$value_arr = array_map('trim',$value_arr);
	$formvars = array_filter($value_arr);
	preg_match_all('/<input type="hidden" name="formhash" id="formhash" value="(.*?)" \/>/is', $content, $matches, PREG_SET_ORDER);
	$formhash = $matches[0][1];
	if(!$formhash) return;
	$submit_url = $host.'forum.php?mod=post&action=reply&fid='.$fid.'&tid='.$tid.'&extra=page%3D1%26filter%3Dtypeid%26typeid%3D1&replysubmit=yes&infloat=yes&handlekey=fastpost';
	$snoopy = get_snoopy_obj($snoopy_args);
	$snoopy->maxredirs = $max_redirs ? $max_redirs : $pick_config['max_redirs'];   
	...
	$formvars["message"] = '{:2_'.rand(25,40).':}路过，来看看，支持楼主{:3_'.rand(40,60).':}';
	$formvars["subject"] = '';
	$formvars["posttime"] = $_G['timestamp'];
	$formvars["formhash"] = $formhash;
	$snoopy->read_timeout = 15;
	$snoopy->submit($action,$formvars);//$formvars为提交的数组
	//print_r($formvars);//exit();
	$snoopy->submittext($submit_url, $formvars); //提交后只返回 去除html的 文本
	$content = $snoopy->results;
	if(strexists($content,$host) && strlen($content) < (strlen($host) + 5)) {
		$snoopy = get_snoopy_obj($snoopy_args);
		$snoopy->fetch($page_url);
		$reply_text = $snoopy->results;
		$html = get_htmldom_obj($reply_text);
		if(!$html) return str_replace($lock_str, '', $str);
		$div = $html->find('div.showhide');
		$showhide = str_iconv($div[0]->innertext);
		$showhide = str_replace('<h4>本帖隐藏的内容</h4>', '', $showhide);
		$str = str_replace($lock_str, $showhide, $str);
		if($cid) {
			DB::update('strayer_article_content', array('content' => paddslashes($str)), array('cid' => $cid));
		}
	}
	return $str;
}
		...
		foreach($temp as $key => $value) {
			...
			foreach($search_arr as $v){
				if(!filter_something($value[0], array($v))){
					preg_match_all("/\<img.+".$evo_img[$v]."=('|\"|)?(.*)(\\1)(.*)?\>/isU", $value[0], $img_arr, PREG_SET_ORDER);
					$value[1] = $img_arr[0][2];
				}
			}
			$search[$key] = $value[0];
			...
			if(in_array(md5($value[0]), $file_img)) {
				$replace[$key] = str_replace('file=', 'src=', $replace[$key]);
			}
		}
	}
;
