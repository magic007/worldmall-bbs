...
function article_edit(){
	...
	if($_GET['submit']){
		...
		cpmsg(milu_lang('save_success', array('msg' => $msg)).'<br><br><a href="?'.PICK_GO.'picker_manage&myfunc=article_edit&aid='.$aid.'&pid='.$pid.'">'.milu_lang('continue_edit').'</a>&nbsp;<span class="pipe">|</span>&nbsp;<a href="?'.PICK_GO.'picker_manage&myac=article_manage&p=1&pid='.$pid.'&status='.$status.'">'.milu_lang('return_list').'</a>'.$article_view_output, PICK_GO.'picker_manage&myfunc=article_edit&aid='.$aid.'&pid='.$pid, 'succeed');
	}else{
		$pid = intval($_GET['pid']);
		$p_arr = get_pick_info($pid);
...
function downremotefile($arr,$action, $old_arr = array()){
		...
		if(is_array($temp) && !empty($temp)) {
			...
			if($arrayimageurl) {
				foreach($arrayimageurl as $key => $tempvalue) {
					...
					$imagereplace['oldimageurl'][] = $tempvalue[4] == 1 ? $tempvalue[0] : $tempvalue[2];
					...
					if(preg_match('/^(http:\/\/|\.)/i', $imageurl)) {
						...
						if(is_array($content_re)){
							...
							$file_name = $attach['name'] = $content_re['file_name'];
							$attach['ext'] = trim($upload->fileext($file_name));
						}else{
							$content = $content_re;
						}
	...
	if(strexists(DISCUZ_VERSION, '2.5')){//2.5版本
		$arr['content']  = dstripslashes($arr['content']);//不必转义
	}
	//print_r($arr['content']);exit();
...
function article_import($action, $args){
	...
	foreach($aid_arr as $k => $aid){
		...
		$today_public_num ++;
		if($p_arr['is_public_del'] != 1) DB::update('strayer_article_title', $setarr, array('aid' => $aid));
...
function article_timing_add($args){
	...
	if($check){
		...
		$aid = DB::insert('strayer_timing', $args, TRUE);
	}
}
...
function article_move_forums($arr, $old_arr){
	...
	if(strexists(DISCUZ_VERSION, '2.5')){//2.5版本 2.5版本多了一个position字段
...
function article_move_portal($arr, $old_arr){
	...
	if($old_arr['portal_id']){//检查文章被放进回收站或者删除
		...
		while ($value = DB::fetch($query)) {
			@unlink($_G['setting']['attachdir'].'/portal/'.$value['filename']);
		}
...
function album_article_content($arr){
	...
	if($arr['pic']) DB::update("home_blog", array('picflag' => 1), array('blogid' => $arr['aid']));	
	$arr['content'] = dstripslashes($arr['content']);
	$arr['content'] =  media_htmlbbcode($arr['content'], $arr['page_url'], 'blog');
	if(!strexists(DISCUZ_VERSION, '2.5')){//2.0版本
		...
		foreach($temp as $key => $value) {
			...
			if(strlen($imageurl)) {
				...
				if(!isset($existentimg[$hash])) {
					...
					//print_r($setarr);
					$setarr['aid'] = DB::insert('forum_attachment', $set_att, true);
					$at[] = $setarr['aid'];
