...
function js_base64_encode($arr){
	foreach((array)$arr as $k => $v){
		if(GBK) $v = diconv($v, 'GB2312', 'UTF-8');
...
function pstripslashes($data){
	if(!strexists(DISCUZ_VERSION, '2.5')) return $data;
...
function get_contents($url, $args = array()){
	...
	//$url = iconv("gbk", "UTF-8", $url);//对于有中文的地址来说，有必要这样处理
	extract($args);
	global $_G;
...
function dz_attach_format($url, $message){
	...
	$attach_arr = array();
	preg_match_all('/<ignore_js_op>\s+<img\s+src="static\/image\/filetype\/(.*?)"[\s\S]*?<a href=[\'"](.*?forum\.php\?.*?)[\'"].*?target="_blank"\>(.*?)<\/a>[\s\S]*?<\/ignore_js_op>/is', $message, $attach_arr, PREG_SET_ORDER);//DZ2.0和DZ2.5的附件
...
function clear_log($del = 0){
	...
	foreach($file_list as $k => $v){
		if(TIMESTAMP - $v['change'] > 3600*24*7) @unlink($v['location']);
	}
