...
switch($myaction){
	case '':
		...
		foreach($cat_arr as $k_c => $v_c){
			$query = DB::query("SELECT * FROM ".DB::table('strayer_picker')." WHERE pick_cid='$v_c[cid]' ORDER BY displayorder ASC,dateline DESC");
...
function article_manage(){
	...
	if($_GET['is_public_del'] && !$info['pick']['is_public_del']) {
		$info['pick']['is_public_del']  = 1;
	}else if(!$_GET['is_public_del'] && $info['pick']['is_public_del']){
		$info['pick']['is_public_del']  = 0;
	}
	...
	$info['forumselect'] = '<select id="forums" name="forums" onchange="getthreadtypes(this.value, 0)">'.forumselect(FALSE, 0, $info['public_class'][0], TRUE).'</select><span id="threadtypes">'.$info['threadtypes'].'</span>';
...
function article_batch(){
	...
	if(!VIP){
		...
		if($today_arr['day'] != date('md', $_G['timestamp'])){
			...
			if($article_public_num > 100) cpmsg_error(milu_lang('article_public_limit', array('n' => 100)));
		}
	}
	extract($_GET);
	extract($set);
